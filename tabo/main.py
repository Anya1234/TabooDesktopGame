...
nextim = pygame.image.load('images/Buttons/next.PNG').convert_alpha()
...
# stars
stars = list(map(pygame.image.load, ['images/StarsImage/stars.PNG',
                                                    'images/StarsImage/stars1.PNG',
                                                    'images/StarsImage/stars2.PNG',
                                                    'images/StarsImage/stars3.PNG']))
for i in stars:
      i.convert_alpha()
...
def one_round(difficulty):
    score = 0
    nextb = Button(WIDTH - nextim.get_width(), HEIGHT - nextim.get_height(), screen, nextim)
    skipb = Button(WIDTH - nextim.get_width() - skip.get_width() - 20,
                   HEIGHT - nextim.get_height(),
                   screen, skip)
    if difficulty == 1:
        words = words1
    else:
